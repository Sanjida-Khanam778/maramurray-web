...
import { useAddPlantMutation, useGetPlantDetailQuery, useEditPlantMutation } from "../Api/plantsApi";

export default function AddPlantForm({ initialData, onBack }) {
  const { data: detailResponse, isLoading: isDetailLoading } =
    useGetPlantDetailQuery(initialData?.id, {
      skip: !initialData?.id,
    });
  ...
  const [addPlant, { isLoading: isAdding }] = useAddPlantMutation();
  const [editPlant, { isLoading: isEditing }] = useEditPlantMutation();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (initialData?.id) {
        const res = await editPlant({ id: initialData.id, formData: fd }).unwrap();
        ...
        <div className="flex items-center gap-3 mb-8 pb-6 border-b border-gray-100">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">
              {initialData ? "Edit Plant" : "Add Plant"}
            </h1>
              ...
              <label className="block text-xs font-medium text-gray-600 mb-1.5 flex items-center gap-1.5">
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M12 3v1.5m0 16.5V22.5M4.5 12H3m19.5 0H21m-3.75-7.5 1.06 1.06M6.75 17.25l-1.06 1.06m0-10.5L6.75 6.75m10.5 10.5 1.06 1.06M12 7.5a4.5 4.5 0 1 0 0 9 4.5 4.5 0 0 0 0-9Z"
                  />
                </svg>
                Spacing
              </label>
